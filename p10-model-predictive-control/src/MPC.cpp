...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    fg[0] = 0; // the cost will be stored in this position
    ...
    for (size_t t = 0; t < N - 1; t++) {
      ...
      const unsigned int t_1 = t + 1;
      AD<double> x1 = vars[x_start + t_1];
      AD<double> y1 = vars[y_start + t_1];
      AD<double> psi1 = vars[psi_start + t_1];
      AD<double> v1 = vars[v_start + t_1];
      AD<double> cte1 = vars[cte_start + t_1];
      AD<double> epsi1 = vars[epsi_start + t_1];
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (i = 1; i < N; i++) {
    predicted_trajectory_x[i-1] = solution.x[x_start + i];
    predicted_trajectory_y[i-1] = solution.x[y_start + i];
  }

  predicted_trajectory_x_ = predicted_trajectory_x;
  predicted_trajectory_y_ = predicted_trajectory_y;
